...
### Узнать курс
###
  async def check_course(course: str,update: Update, context: ContextTypes.DEFAULT_TYPE):
      await context.bot.send_message(chat_id=update.effective_chat.id, text=course)

###
### Изменить курс
###
  async def change_course(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
### Маржа
###
  async def marje(update: Update, context: ContextTypes.DEFAULT_TYPE):
      await context.bot.send_message(chat_id=update.effective_chat.id, text="Маржа: ", reply_markup=admin.marje)

###
### Узнать маржу
###
  async def check_marje(course: str,update: Update, context: ContextTypes.DEFAULT_TYPE):
      await context.bot.send_message(chat_id=update.effective_chat.id, text=course)
...
###
 ...
 ### СТАТИСТИКА ###
  ...
  async def users(users, update: Update, context: ContextTypes.DEFAULT_TYPE):
      # for i in users:
      #     await
      ##TODO взять из
      print(users)

  async def orders(orders, update: Update, context: ContextTypes.DEFAULT_TYPE):
      for i in orders:
          print(orders)

  async def revenue(summ, update: Update, context: ContextTypes.DEFAULT_TYPE):
